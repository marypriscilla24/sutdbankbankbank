...
public class RegisterServlet extends DefaultServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//clientAccount.setAddress(filterXSS(request.getParameter("address")));
	...
	/*
	//ngpc
	public void filterXSS(String gotXSSinput) {
		String regex = "<\\s*script\\s*>|alert\\s*\\(\\s*\\)";
		Pattern p = Pattern.compile(regex);
		Matcher m = p.matcher(gotXSSinput);
		if (m.find()) {
			//Error handling
			//logger write
			//System.out.println("#1  " + gotXSSinput);
		} else {
			//Error handling
			//logger write
			//System.out.println("#2  " + gotXSSinput);
		}
	}//ngpc
	*/
